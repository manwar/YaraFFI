...
if ($@) {
    die "Could not locate libyara via system or Alien::YARA fallback: $@\n";
}
...
sub DESTROY {
    ...
    # Skip if Perl is in global destruction phase
    return if ${^GLOBAL_PHASE} eq 'DESTRUCT';

    if (my $rules = delete $self->{rules}) {
...
These features use L<FFI::Platypus::Record> to safely access C<YARA> internal structures,
